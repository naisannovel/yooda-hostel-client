...
console.log(spinner);
    ...
    if( foundedStudentList?.length ){
        displayFoundedStudentList = foundedStudentList?.map(item =>(
            <div onClick={()=> foundedStudentClickHandler(item)} key={item?._id} className="py-2 bg-blue-100 border-b-2 border-l border-r rounded hover:bg-blue-50 hover:border-r-0 hover:border-l-0 cursor-pointer">
            ...
            <div className="container flex items-start mt-4">
                    <div className='w-8/12'>
                        ...
                        <div className='mt-4 flex flex-wrap justify-center'>
                            {
                                foodList?.map(item =><div className='basis-48 shrink-0'> <input {...register("food", { required: true })} type="checkbox" id={item?._id} value={item?.name} /> <label className='text-lg font-semibold text-blue-900 ml-1' htmlFor={item?._id}>{ item?.name }</label> </div>)
                            }
                        </div>
                    </div>
                <div className='w-4/12'>
                    <div>
                        <Calendar className='border-2 rounded-lg' onChange={setDate} value={date} />
                    </div>
                </div>
            </div>
            <div className='text-center mt-8 mb-20'> <button type='submit' disabled={!(studentValue?._id !== undefined)} className='py-2 px-12 rounded-lg bg-blue-200 disabled:bg-blue-100 hover:bg-blue-200 text-blue-800 disabled:cursor-not-allowed disabled:text-gray-400 text-lg font-bold focus:outline-none focus:ring-2'>Submit</button> </div>
            </form>
            </div>
    );
};
