...
const AddStudentDrawer = ({ isOpen, drawerOpenHandler, addNewStudent, setSpinner }) => {
  ...
  const onSubmit = (data) => {
    ...
    axios.post(`${MAIN_API}/student`, data, {
      ...
      .then((response) => {
        addNewStudent((prevState) => [response.data, ...prevState]);
        drawerOpenHandler();
        setSpinner(false);
  ...
  return (
    <div className="w-full h-full relative">
      <div
        className={`fixed overflow-y-scroll w-2/6 h-full bg-white shadow-2xl shadow-gray-400 rounded-tl-2xl top-0 right-0 z-50 transition duration-1000 ease-in-out ${ isOpen ? "" : "translate-x-full" }`}>
